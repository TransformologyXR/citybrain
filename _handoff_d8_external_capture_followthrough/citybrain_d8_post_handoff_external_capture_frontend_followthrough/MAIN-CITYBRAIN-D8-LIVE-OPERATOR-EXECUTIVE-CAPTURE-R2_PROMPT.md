...
Stage 3 of 20 in the D8 post-handoff follow-through sequence.

## Purpose

Run the live local surfaces and capture operator and executive walkthrough media. Actual media files must be copied into the output root or referenced by stable local paths and hashed.
...
2. Produce the stage artifact root under `outputs/main_citybrain_d8_live_operator_executive_capture_r2`.
...
- `D8_LIVE_OPERATOR_EXECUTIVE_CAPTURE_R2_DECISION.json`
- `D8_OPERATOR_CAPTURE_MANIFEST.json`
- `D8_EXECUTIVE_CAPTURE_MANIFEST.json`
- `CAPTURE_FILE_HASHES.json`
- `HASH_MANIFEST.json`
