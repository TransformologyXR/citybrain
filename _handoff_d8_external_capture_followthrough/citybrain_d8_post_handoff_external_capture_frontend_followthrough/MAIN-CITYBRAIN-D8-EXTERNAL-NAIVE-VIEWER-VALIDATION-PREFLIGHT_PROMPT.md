...
Stage 7 of 20 in the D8 post-handoff follow-through sequence.

## Purpose

Prepare external viewer validation. Require consent/anonymous IDs only; no sensitive personal data. Check whether viewer-session input files exist; if not, create templates and mark pending, not green.
...
2. Produce the stage artifact root under `outputs/main_citybrain_d8_external_naive_viewer_validation_preflight`.
...
- `D8_EXTERNAL_NAIVE_VIEWER_VALIDATION_PREFLIGHT_DECISION.json`
- `VIEWER_INPUT_TEMPLATE.json`
- `VIEWER_PRIVACY_AND_CONSENT_NOTE.md`
- `VIEWER_SESSION_INPUT_INVENTORY.json`
- `HASH_MANIFEST.json`
