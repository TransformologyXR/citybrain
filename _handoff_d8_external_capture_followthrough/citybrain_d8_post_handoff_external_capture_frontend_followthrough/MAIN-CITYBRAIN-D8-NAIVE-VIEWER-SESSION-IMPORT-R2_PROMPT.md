...
Stage 9 of 20 in the D8 post-handoff follow-through sequence.

## Purpose

Import manually collected external viewer session records from a local input folder. Do not invent records. Validate schema, anonymize viewer IDs, and preserve raw vs normalized evidence.
...
2. Produce the stage artifact root under `outputs/main_citybrain_d8_naive_viewer_session_import_r2`.
...
- `D8_NAIVE_VIEWER_SESSION_IMPORT_R2_DECISION.json`
- `VIEWER_SESSION_IMPORT_REPORT.json`
- `NORMALIZED_VIEWER_RESPONSES.json`
- `VIEWER_EVIDENCE_HASHES.json`
- `HASH_MANIFEST.json`
