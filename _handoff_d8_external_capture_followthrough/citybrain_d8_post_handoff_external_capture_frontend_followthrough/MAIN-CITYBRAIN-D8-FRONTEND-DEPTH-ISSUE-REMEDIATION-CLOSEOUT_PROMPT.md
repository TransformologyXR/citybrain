...
Stage 17 of 20 in the D8 post-handoff follow-through sequence.

## Purpose

Close front-end remediation if P0 issues are zero and P1 issues are either fixed or accepted with reason. Produce patch log and remaining issue backlog.
...
2. Produce the stage artifact root under `outputs/main_citybrain_d8_frontend_depth_issue_remediation_closeout`.
...
- `D8_FRONTEND_DEPTH_ISSUE_REMEDIATION_CLOSEOUT_DECISION.json`
- `FRONTEND_CLOSEOUT_SUMMARY.json`
- `FRONTEND_REMAINING_ISSUE_BACKLOG.json`
- `HASH_MANIFEST.json`
