...
**Acceptance gate (target):** G4 (from the original roadmap)
**Document purpose:** Define the next credible product finish line *after* the Application Snapshot. Platform v1 is what makes TXR City Brain a *platform* rather than a *demonstration* — a system that can be deployed for a new flow or a new city with bounded effort, that exercises every layer of the architecture at least once on a connected runtime path, and that surfaces multiple cognition modes and personas through the same governance discipline.

**How to maintain this document:** This is a target. Each criterion has a clear test so the bar does not drift. When all criteria pass, this doc's status flips from "target" to "achieved-on-{date}" and items move into doc 01. Items that are *beyond* Platform v1 live in doc 04 — the boundary between this doc and doc 04 is the load-bearing line that prevents over-scoping.
...
## 0. D19/D20 actuator / LLM / control boundary

PV1-D19/D20 owns the guardrail/action-policy boundary for control. No LLM controls actuators. No model emits operational commands. No review route becomes control. Platform v1 may render evidence, propose review-only actions, and require HITL approval states, but any actuator-adjacent adapter is deferred to a future safety-critical program with deterministic policy enforcement, certification, and a formal safety case.

This boundary is part of Platform v1 governance, not a post-PV1 autonomy lane.

---

## 1. The seven completion criteria

Platform v1 is done when all seven of these hold:
...
| Flow 3 | Resilience / fire-incident context extension | London | Accepted as LON-F3X through LON-FLOWX-D6; review-only, no emergency command or dispatch |
| Flow 4 | Mobility / environment extension | London | Accepted as LON-F4X through LON-FLOWX-D6; review-only, no route guarantee or public-order instruction |
| Flow 5 | Flood / climate-risk extension | London | Accepted as LON-F5X through LON-FLOWX-D6; risk context only, no utility control or emergency response |
...
**Most recent mounted expansion lane:**

| Expansion gate | Domain | City core | Current status |
|---|---|---|---|
| `LON-F3X-D2` | Resilience / fire-incident context | London | `PASS_SOURCE_LANDING_JOIN_HARDENING` |
| `LON-F4X-D1/D2` | Mobility / environment | London | `PASS_SOURCE_SCOUT` + `PASS_SOURCE_LANDING_JOIN_HARDENING` |
| `LON-F5X-D1/D2` | Flood / climate risk context | London | `PASS_SOURCE_SCOUT` + `PASS_SOURCE_LANDING_JOIN_HARDENING` |
| `LON-FLOWX-D3/D4/D5/D6` | EvidenceBundles, replay/live proof, hero/freeze, accepted snapshot | London | `PASS_ACCEPTED_EXTENSION_SNAPSHOT` |

**Why this matters:** The platform has enough accepted breadth to stop rebuilding city cartridges. `LON-F3X`, `LON-F4X`, and `LON-F5X` prove the city-core + flow-sub-cartridge model on one accepted London core: each extension adds its own source ledger, joins, EvidenceBundles, replay/live proof, hero/freeze package, and D6 snapshot while inheriting the London core IDs, geography, face/NIM conventions, and limitations.
...
**Honest framing:** London F3X/F4X/F5X are accepted as bounded review-context sub-cartridges, not emergency, dispatch, public-order, utility-control, health, legal, routing, or certified affected-asset systems. They inherit London's IDs, geography, evidence style, face/NIM conventions, and limitations, but each earned its own D2-D6 gate sequence.
...
When all four tests pass, Platform v1 is done.
