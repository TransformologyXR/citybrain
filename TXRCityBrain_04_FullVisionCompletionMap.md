...
| Per-flow sub-cartridge adapters for London Flow 3/4/5 and future Flow 6 | `[I]` for London F3X/F4X/F5X through `LON-FLOWX-D6`; `[P]` for future Flow 6 | Platform v1 → Full Vision |
...
| 3X-London | Resilience / fire-incident context expansion | Public safety + mobility + environment | `LON-F3X-D1` | `[I]` source/API scout green; D2 required before sub-cartridge acceptance | Platform v1 |
...
**Slotting correction:** The long-horizon map now distinguishes **city cores** from **flow sub-cartridges**. London is accepted today as Flow 2, not Flow 3/4/5. `LON-F3X-D1` is green as a source/API scout; the right next move is `LON-F3X-D2`, plus `LON-F4X-D1` and `LON-F5X-D1`, each with its own source ledger, joins, EvidenceBundles, replay/live proof, and hero/freeze gate.
...
34. `LON-F3X-D1` London resilience / fire-incident context scout — done
...
## 10. What this document is for, and what it is not for
...
## 11. Post-PV1 strategic fork and platform gap map

This section is a fenced post-PV1 roadmap note. It is not an active Codex build lane, not a city/data expansion trigger, and not an Omniverse implementation start.

### Current two-track status

Track 1 - PV1 Main:
- Active/final lane for this note: PV1-D19/D20/D21/D22 - Guardrail / Action Policy / Composite Platform v1 Snapshot.
- Status language: final review-only platform snapshot block pending, in progress, or subject to its own gate. This document does not mark PV1 complete.

Track 2 - XDATA / City-Flow Expansion:
- Closed as review-route-ready and data-route-catalogued.
- No new accepted flows.
- Future Track 2 work requires an explicit trigger: a stable XDATA bulk sweep, Singapore LTA/auth recovery, or an acceptance-policy gate.

### Platform gaps before more city or data expansion

Before starting more cities or broad data expansion, the following gaps remain missing or not-yet-productized:

1. Final PV1 guardrail/action policy plus composite snapshot.
2. Runtime data-route API contract.
3. Omniverse / USD scene-binding contract.
4. Source freshness / incremental update layer.
5. Review-route product surface / control-room UX hardening.
6. Acceptance policy for review-route-ready flows.
7. Production review-only deployment layer: RBAC, observability, SLOs, backups, secrets.
8. Post-PV1 narrative/product packaging.

### Visual / Omniverse asset lane boundary

The user may manually download, build, buy, or source grey-city and textured hero-location assets for NYC, London, Chicago, and Barcelona. The goal is globally geolocated Omniverse/USD-ready city substrates. This is manual asset preparation, not an active Codex build lane and not evidence that the visual twin is complete or integrated.

A later CITY-SCENE-BINDING-D1 contract should define:
- city CRS / coordinate system
- global geolocation anchor
- USD prim path conventions
- city/flow/entity-to-asset IDs
- hero-location IDs
- review-route overlay schema
- time/replay overlay schema
- evidence panel binding
- asset provenance/licensing

### Autonomy boundary

Autonomy is not a PV1 continuation. Autonomy is a deferred safety-critical future program. No LLM controls actuators. No model emits operational commands. No review route becomes control. Only deterministic, policy-bound, certified adapters may ever sit near actuators, and only inside a future safety case.

### Recommended post-PV1 order

This is a recommendation, not an active lane:

1. Product/demo/narrative pack.
2. Compliance/planning vertical depth pack.
3. Omniverse visual twin / scene-binding.
4. Production review-only hardening.
5. Data moat / XDATA sweep.
6. Agent/NIM/NeMo briefing layer.
7. Autonomy, deferred as safety-critical future program.

Final sequencing should be chosen based on data strength, platform breadth, asset readiness, and target audience/event.

Allowed language: post-PV1 option, future lane, manual asset preparation, fenced roadmap, platform gap, review-only baseline.

Forbidden overclaims: PV1 complete before D19-D22 passes; Omniverse lane active in Codex; autonomy is next; review-route-ready means accepted; grey-city assets already integrated; visual twin complete.

---

## 12. The honest framing for the outer horizon

> "The Application Snapshot (doc 02) is a real, governed, citywide, end-to-end NYC + London demonstration that earns its claims. Platform v1 (doc 03) is the next credible product finish line — 2–3 cities, 3–4 flows, multi-mode cognition, one simulator, four personas, live event fabric, HITL approval. Full Vision (this doc) is the architecture's outer ambition with every named NVIDIA component, every flow, every simulator, every advanced frontier capability. The first is shippable. The second is buildable. The third is mappable — and we maintain it honestly so nothing in the architecture goes invisibly unbuilt."
