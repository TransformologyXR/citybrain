    ...
    for domain in PRIORITY_DOMAINS:
        ...
        for city in cities:
            if city == "cross_city_or_product" and domain != "data_quality_maturity":
                continue
            city_domain = (by_city.get(city, {}).get("domains", {}) or {}).get(domain, {})
            rows.append({
                "selection_id": stable_id(domain, city, prefix="seed_r3_selection"),
                "city": city,
                "domain": domain,
                "priority": entry.get("priority", "P0"),
                "landed_rows": as_int(city_domain.get("landed_rows") if city_domain else entry.get("landed_rows")),
                "landed_features": as_int(city_domain.get("landed_features") if city_domain else entry.get("landed_features")),
                "registered_or_total_count": as_int(
                    city_domain.get("registered_or_total_count") if city_domain else entry.get("registered_or_total_count")
                ),
                "ledger_rows": as_int(city_domain.get("ledger_rows") if city_domain else entry.get("ledger_rows")),
