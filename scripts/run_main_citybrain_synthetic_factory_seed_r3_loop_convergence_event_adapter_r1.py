...
CANONICAL_LOOP_FAMILY_IDS = list(LOOP_FAMILY_ALIAS_MAP.values())
...
class InputSummary:
    ...
    product_brief_rows: int
    product_spatial_rows: int
...
def summarize_inputs(preflight: Path, product: Path, refresh: Optional[Path]) -> InputSummary:
    ...
    brief_rows = count_jsonl(product / "BRIEF_FIXTURE_INDEX_R1.jsonl")
    spatial_rows = count_jsonl(product / "SPATIAL_OVERLAY_INDEX_R1.jsonl")
    return InputSummary(
        ...
        product_brief_rows=brief_rows,
        product_spatial_rows=spatial_rows,
    ...
    for family in LOOP_FAMILIES:
        for layer in TRUTH_LAYERS:
            ...
            rows.append({
                ...
                "loop_family": family,
                "canonical_loop_family_id": LOOP_FAMILY_ALIAS_MAP[family],
...
def validate_output(out: Path) -> Dict[str, Any]:
    ...
    for path in out.glob("*.jsonl"):
        try:
            ...
            jsonl_errors.append({"path": path.name, "error": str(exc)})

    decision = load_json(out / "SEED_R3_LOOP_CONVERGENCE_EVENT_ADAPTER_DECISION.json", {}) or {}
    alignment = load_json(out / "LOOP_FAMILY_ALIGNMENT_REPORT.json", {}) or {}
    secret = load_json(out / "SECRET_SCAN_REPORT.json", {}) or {}
    hash_errors = []
    manifest = load_json(out / "HASH_MANIFEST.json", {}) or {}
    ...
    validated = (
        not missing
        and not json_errors
        and not jsonl_errors
        and not hash_errors
        and decision.get("status") == STATUS
        and secret.get("status") == "PASS"
        and alignment.get("canonical_family_alias_map") == LOOP_FAMILY_ALIAS_MAP
    )
    return {
        "validated": validated,
        ...
        "event_adapter_rows": decision.get("event_adapter_rows"),
        "canonical_family_alias_map": alignment.get("canonical_family_alias_map"),
    }
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    canonical_family_counts: Dict[str, int] = {fam: 0 for fam in CANONICAL_LOOP_FAMILY_IDS}
    resolution_counts: Dict[str, int] = {}
    for row in adapter_rows:
        family_counts[row["loop_family"]] += 1
        canonical_family_counts[row["canonical_loop_family_id"]] += 1
    ...
    write_json(out / "LOOP_FAMILY_ALIGNMENT_REPORT.json", {
        ...
        "canonical_family_alias_map": LOOP_FAMILY_ALIAS_MAP,
        "canonical_family_ids": CANONICAL_LOOP_FAMILY_IDS,
        "family_counts": family_counts,
        "canonical_family_counts": canonical_family_counts,
        "all_required_families_present": all(family_counts[f] > 0 for f in LOOP_FAMILIES),
        "all_canonical_families_present": all(canonical_family_counts[f] > 0 for f in CANONICAL_LOOP_FAMILY_IDS),
    ...
    product_readiness = {
        "status": "PASS" if all([
            summary.product_watch_rows > 0,
            summary.product_check_rows > 0,
            summary.product_brief_rows > 0,
            summary.product_spatial_rows > 0,
        ]) else "PARTIAL",
        "readiness_scope": "adapter can be replayed through Event Fabric before downstream WATCH/CHECK/BRIEF/SPATIAL materialization",
        "direct_fixture_bypass_allowed": False,
        "source": str(product),
        "counts": {
            "WATCH": summary.product_watch_rows,
            "CHECK": summary.product_check_rows,
            "BRIEF": summary.product_brief_rows,
            "SPATIAL": summary.product_spatial_rows,
            "EVENT_REPLAY": summary.product_event_rows,
        },
        "surface_readiness": {
            "WATCH": summary.product_watch_rows > 0,
            "CHECK": summary.product_check_rows > 0,
            "BRIEF": summary.product_brief_rows > 0,
            "SPATIAL": summary.product_spatial_rows > 0,
        },
        "limitation": "These are prior product-consumption fixture counts used as readiness context; Seed R3 adapter rows must still enter through Event Fabric.",
    }
    write_json(out / "PRODUCT_CONSUMPTION_READINESS_R1.json", product_readiness)
    ...
    write_json(out / "LOOP_FAMILY_ALIAS_MAP_R1.json", {
        "status": "PASS",
        "purpose": "Prevent Seed R3 adapter output from creating a third product-loop naming universe.",
        "package_loop_families": LOOP_FAMILIES,
        "canonical_family_ids": CANONICAL_LOOP_FAMILY_IDS,
        "alias_map": LOOP_FAMILY_ALIAS_MAP,
        "emitted_fields": ["loop_family", "canonical_loop_family_id"],
    })
    ...
    write_json(out / "EVENT_FABRIC_INGESTION_CONTRACT_R1.json", {
        ...
        "required_fields": [
            ...
            "canonical_loop_family_id", "event_time", "processing_time", "truth_layer", "source_class", "donor_refs",
            "synthetic_truth_ref", "candidate_entity_refs", "payload", "expected_resolution_state",
            "limitation_refs"
        ],
        "allowed_loop_families": LOOP_FAMILIES,
        "canonical_family_alias_map": LOOP_FAMILY_ALIAS_MAP,
        "allowed_canonical_loop_family_ids": CANONICAL_LOOP_FAMILY_IDS,
    ...
    decision = {
        ...
        "family_counts": family_counts,
        "canonical_family_counts": canonical_family_counts,
        "watch_check_brief_spatial_consumption_readiness": product_readiness,
        "limitations": [
            "local/replay only",
    ...
    (out / "CODEX_CLOSEOUT.md").write_text(
        ...
        "Canonical family aliases are emitted in `LOOP_FAMILY_ALIAS_MAP_R1.json` and each row's `canonical_loop_family_id`.\n\n"
