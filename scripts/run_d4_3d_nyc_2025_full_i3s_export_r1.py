#!/usr/bin/env python3
"""D4-3D-NYC-2025-FULL-I3S-EXPORT-R1.

Full Codex-side export of Buildings 3D NYC 10 27 2025 from public I3S
SceneServer resources. Exports leaf-node geometry as sharded USDA and selected
identity attributes as JSONL sidecars. No ArcGIS Pro manual export required.
"""
...
TASK = "D4-3D-NYC-2025-FULL-I3S-EXPORT-R1"
PASS_FULL = "PASS_D4_3D_NYC_2025_FULL_I3S_EXPORT_R1"
PASS_LIMITED = "PASS_D4_3D_NYC_2025_FULL_I3S_EXPORT_R1_WITH_LIMITATIONS"
FAIL = "FAIL_D4_3D_NYC_2025_FULL_I3S_EXPORT_R1"
OUT = Path("outputs/d4_3d_nyc_2025_full_i3s_export_r1")
...
PREVIOUS_ROOTS = {
    ...
    "nyc_exportability": Path("outputs/d4_3d_nyc_arcgis_exportability_batch_r1"),
...
def nice_value(name: str, value: Any) -> Any:
    if isinstance(value, float) and name.lower() in {"bin", "doitt_id", "lod", "heightft", "cnstrct_yr"}:
        if value.is_integer():
            return int(value)
    return value
...
def build_identity_rows(pkg: dict[str, Any], attr_names: list[str]) -> list[dict[str, Any]]:
    ...
    for i in range(feature_count):
        row = {
            ...
            "citybrain_identity_status": "BIN_BBL_DOITT_SOURCE_ID_CANDIDATE",
        ...
        row["citybrain_building_id"] = f"nyc:building:bin:{row.get('bin')}" if row.get("bin") not in {None, "", 0} else None
        bbl = row.get("base_bbl") or row.get("mpluto_bbl")
        row["citybrain_parcel_id"] = f"nyc:parcel:bbl:{bbl}" if bbl not in {None, "", 0} else None
        row["citybrain_3d_source_id"] = f"nyc:doitt_3d_building:{row.get('doitt_id')}" if row.get("doitt_id") not in {None, "", 0} else None
        rows.append(row)
    return rows
...
(
    ...
    metersPerUnit = 1
    upAxis = "Z"
    doc = "CityBrain NYC 2025 Buildings full I3S export shard {shard_id:03d}. Leaf-node geometry only."
)
...
def write_shard(
    ...
    shard_path = OUT / "usd_shards" / f"NYC_2025_LOD2_shard_{shard_id:03d}.usda"
...
def write_master_usda(shard_reports: list[dict[str, Any]]) -> Path:
    master = OUT / "NYC_2025_BUILDINGS_FULL_MASTER.usda"
    refs = []
...
(
    ...
    metersPerUnit = 1
    upAxis = "Z"
    doc = "CityBrain full sharded export of Buildings 3D NYC 10 27 2025. References shard USDA files."
)
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_json(OUT / "NYC_2025_I3S_LAYER_METADATA.json", {"root": root, "layer": layer})
    write_json(OUT / "NYC_2025_LEAF_NODE_INVENTORY.json", {"inventory": inventory, "leaf_nodes": [leaf.__dict__ for leaf in leaves]})
    write_json(OUT / "NYC_2025_SELECTED_ATTRIBUTE_MAP.json", {"selected_fields": attr_names, "attribute_storage": attr_infos})
    ...
    write_text(
        OUT / "README.md",
        f"# {TASK}\n\nStatus: `{status}`\n\nFull Codex-side sharded export of Buildings 3D NYC 10 27 2025 from I3S leaf nodes. Master scene: `{master.as_posix()}`.\n",
    )
    write_text(
        OUT / "NYC_2025_FULL_EXPORT_REPORT.md",
        f"# NYC 2025 Full I3S Export\n\n"
        f"Status: `{status}`\n\n"
        f"- Source: {URL}\n"
        ...
        "Boundary: 3D geometry and BIN/BBL/DOITT/globalid are review/context identity anchors only; no ownership, legal, enforcement, dispatch, control, or certified affected-building claim.\n",
    )
    decision = {
        "status": status,
        "task_name": TASK,
        ...
        "limitations": [
            ...
            "USD prims are node-level meshes; per-building identity is preserved in JSONL sidecars by feature/face range.",
    ...
    write_json(OUT / "D4_3D_NYC_2025_FULL_I3S_EXPORT_R1_DECISION.json", decision)
    shutil.copy2(Path(__file__), OUT / "run_d4_3d_nyc_2025_full_i3s_export_r1.py")
    hash_outputs()
    return decision
