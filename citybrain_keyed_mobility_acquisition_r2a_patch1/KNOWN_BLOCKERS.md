...
The user confirmed `https://datamall2.mytransport.sg/` resolves in a browser. The assistant sandbox fetch failure is therefore treated as local DNS/fetch environment limitation, not endpoint obsolescence. TfL swagger at `https://api.tfl.gov.uk/swagger/docs/v1` is reachable and confirms app-key query security plus the road, stop point, line, bike point, and air-quality API families.
