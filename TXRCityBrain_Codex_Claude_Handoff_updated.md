...
- Track 2 current status: 7 data-route-ready rows, 5 review-route-ready-not-accepted lanes, 3 candidate-only rows, 0 new accepted flows.
...
1. `FLOWX-REVIEW-FLOW-ACCEPTANCE-D1` as a **post-PV1 addendum** if the five review-route-ready lanes should become `ACCEPTED_REVIEW_FLOW` / `ACCEPTED_REVIEW_FLOW_WITH_LIMITATIONS`.
2. Product/demo/narrative pack.
3. Compliance/planning vertical depth pack.
4. Omniverse visual twin / `CITY-SCENE-BINDING-D1` once assets are ready.
5. Production review-only hardening.
6. Data moat / source freshness layer.
7. Agent/NIM/NeMo briefing layer.
8. Autonomy / operational control remains deferred as a separate safety-critical future program.
