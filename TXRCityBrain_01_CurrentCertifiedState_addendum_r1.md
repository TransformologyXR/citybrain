...
The system now has a green **Platform v1 review-only snapshot**: event fabric, review-only incident/plan modes, SUMO simulator bridge, HITL approval lifecycle, persona renderings, guardrail/action policy, composite snapshot, and final audit have all passed. The system remains non-operational: no emergency dispatch, public-safety instruction, enforcement, utility/traffic/transit/port/airport control, health determination, or certified affected-asset/building decision. Runtime perception, production hardening, Omniverse scene binding, and autonomous control remain outside this certified state and are tracked as post-PV1 / full-vision work.

---


## 1A. Platform v1 review-only snapshot — certified 2026-06-28
...
**What is certified:** a governed, evidence-backed, review-only Platform v1 freeze.

**Green PV1 chain now captured:**

| Block | Status |
|---|---|
| PV1-D3/D4 — cross-city ontology v2 | `PASS_CROSS_CITY_ONTOLOGY_V2` |
| PV1-D5/D6/D7 — file-backed event fabric | `PASS_FILE_BACKED_EVENT_FABRIC` |
| PV1-D8/D9 — review-only Incident + Plan modes | `PASS_REVIEW_ONLY_INCIDENT_AND_PLAN_MODES` |
| PV1-D10/D11/D12 — SUMO simulator bridge | `PASS_SUMO_SIMULATOR_BRIDGE` |
| PV1-D13/D14/D15 — HITL approval lifecycle | `PASS_HITL_APPROVAL_LIFECYCLE` |
| PV1-D16/D17/D18 — persona renderings | `PASS_PERSONA_RENDERINGS` |
| PV1-D19/D20/D21/D22 — guardrail/action policy + composite snapshot | `PASS_PLATFORM_V1_REVIEW_ONLY_SNAPSHOT` |
...
**Boundary:** PV1 is review-only. HITL approval means approved for review/planning use, not execution. Persona rendering does not approve or execute an action. SUMO remains synthetic/demo context. Track 2 review routes remain non-operational and are not accepted flows unless a separate acceptance-policy gate says so.
...
**Not in this document:** Omniverse 3D control room, BIN-tagged USD scene with click-through to graph, multi-agent trace panel as polished product feature, production UI/UX, production deployment, or autonomous control. Personas and HITL are now certified inside PV1 as review-only mechanisms; the cinematic/3D product surface remains post-PV1.

### L9 · Governance — PV1 review-only guardrails certified
...
**PV1 governance now certified:**
- HITL approval object/state and deterministic lifecycle are implemented for review/planning use.
- Audit ledger/hash chain is verified for HITL records.
- D19/D20 consequential-action policy and guardrail harness are green.
- Forbidden operational/control/public-safety/enforcement/health claims are blocked.
- Review-only boundary is explicit across Incident, Plan, SUMO, HITL, personas, and Track 2 review routes.

**Still outside this document:**
- Production workflow integrations.
- Real actuator adapters.
- Autonomous control.
- Formal safety case / shadow-mode operations.
- Monitoring after real-world approved actions, because PV1 does not execute real-world actions.
...
**Slotting update:** Treat NYC, London, and Chicago as accepted **city cores**. New flows mount as city-flow sub-cartridges using the `{CITY}-F{FLOW}X-D{DAY}` convention. For example, `LON-F3X-D1` is a London Flow 3 expansion scout mounted onto the accepted London core, not a fresh London cartridge.

**Honest framing:** London is accepted today as **Flow 2 on a second city**, not as London Flow 3. London Flow 3 requires LFB incidents/mobilisation, TfL disruptions/routes, air quality, vulnerable-asset/population context, and a new resilient-city EvidenceBundle/replay/hero gate. Singapore Flow 4 remains planned until authenticated live API proof clears. Barcelona is a fresh city-core scout candidate.

**Not in this document:** London Flow 3/4/5 sub-cartridges, Singapore Flow 4 accepted proof, Barcelona core, Dubai cartridge, oil & gas cartridge. See docs 03–04 and `TXRCityBrain_CityCore_Subcartridge_Model.md`.
...
- London Flow 3/4/5 sub-cartridges are not accepted yet; they are expansion scouts mounted onto the accepted London core
...
- London Flow 3, London Flow 4, London Flow 5, Singapore Flow 4 accepted proof, and other unlisted flow/city combinations do not exist yet (see doc 03, doc 04, and `TXRCityBrain_CityCore_Subcartridge_Model.md`)
