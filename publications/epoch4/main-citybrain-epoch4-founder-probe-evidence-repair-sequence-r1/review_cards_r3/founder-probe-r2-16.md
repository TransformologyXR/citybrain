# Founder Probe Review Card R3: founder-probe-r2-16

## Task Identity

- Task id: `founder-probe-r2-16`
- Family: `permit_inspection_delay`
- Scenario: `contradiction_pair`
- Eval case: `eval-r2:permit_inspection_delay:contradiction_pair`
...
```json
{
  ...
  "evidence_packet": {
    ...
    "check_v1_result_ref": "check:v1:permit_inspection_delay:simulation_v2_4",
    "data_maturity_refs": [
    ...
    "diff_source_refresh_refs": [
      "diff:v2_4:permit_inspection_delay:history_delta"
    ],
    "event_state_ref": "event_state:v2_4:permit_inspection_delay",
    "family_id": "permit_inspection_delay",
    "limitations": [
      "internal packet only",
      "no founder review session run",
...
```json
{
  ...
  "challenge_boundary_evidence": {
    "case_id": "eval-r2:permit_inspection_delay:contradiction_pair",
    "expected_check_outcome": "downgrade_contradiction",
    "family_id": "permit_inspection_delay",
  ...
  "matched_actual_check_outcome": "contradiction"
}
```
...
CHECK/harness evidence reports contradiction; contradiction should hold or downgrade the claim, not create an action.

### Why CHECK Passed / Downgraded / Abstained / Held

The case contains conflicting evidence. That conflict matters because a founder should see why CityBrain cannot promote the claim without review.
